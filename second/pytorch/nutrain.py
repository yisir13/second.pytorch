...
from second.pytorch.builder import (box_coder_builder, input_reader_builder,
                                      ...
                                      second_builder)
from second.utils.eval import get_official_eval_result
from second.utils.eval_nu import get_ikg_eval_result
    ...
    try:
        for it_loop in range(total_loop):
            ...
            for step in range(steps):
                ...
                global_step = net.get_global_step()-24300
            ...
            gt_annos = []
            for info in eval_dataset.dataset.nusc_infos:
                gt_dic = {}
                gt_dic.update({
                    'name': info['gt_names'] ,
                    'dimensions':info['gt_boxes'][:,3:6] ,
                    'location':info['gt_boxes'][:,:3] ,
                    'rotation_y':info['gt_boxes'][:,6],
                    })
                gt_annos.append(gt_dic)



            results,mAPbev = get_ikg_eval_result(gt_annos, dt_annos, class_names)
            for i, class_name in enumerate(class_names):
                writer.add_scalar('bev_ap:{}'.format(class_name), mAPbev[i], ep)
            writer.add_scalar('bev_map', np.mean(mAPbev[:]), ep)
            print(results)

            writer.add_text('eval_result', results, ep)
            net.train()
...
def predict_nusc_to_anno(net,example,class_names,
                          center_limit_range=None,
                          lidar_input=False,
                          global_set=None):
    '''prediction lidar annos: name , dimensions, location, roration, score, image_idx
    '''
    ...
    for i, preds_dict in enumerate(predictions_dicts):
        ...
        if preds_dict['box3d_lidar'] is not None:
            ...
            if num_example != 0:
                ...
                annos.append(kitti.empty_nu_anno())
        else:
            annos.append(kitti.empty_nu_anno())
...
def evaluate(config_path,
    ...
    if not predict_test:
        ...
        for info in eval_dataset.dataset.nusc_infos:
            ...
            gt_annos.append(gt_dic)
        result, _ = get_ikg_eval_result(gt_annos, dt_annos, class_names)
        print(result)

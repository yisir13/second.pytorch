...
from second.data.preprocess import _read_and_prep_v9, _read_and_prep_nu
...
class NuScenesDataset(Dataset):
    '''Nusc pkl infos: ego2global_rotation,lidar_path,gt_boxes,gt_names,num_lidar_pts...
    lidar points:xyz I lineidx
    '''
    def __init__(self, info_path, root_path,num_point_features,
        ...
        print("remain number of infos:", len(self._nusc_infos))
    ...
    @property
    def nusc_infos(self):
        return self._nusc_infos

    def __getitem__(self, idx):
        return _read_and_prep_nu(
...
class KittiDataset(Dataset):
    '''KITTI DATASET infos: annos(alpha bbox diff groupids index location name occluded rot-y score truncated)
    calib, image_idx, img_path, ima_shape, pointcloud_num_featrues,v_path
    '''
    def __init__(self, info_path, root_path, num_point_features,
                 target_assigner, feature_map_size, prep_func):
        with open(info_path, 'rb') as f:

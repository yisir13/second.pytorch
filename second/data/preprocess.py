...
def prep_pointcloud_nu(input_dict,
                    root_path,
                    voxel_generator,
                    target_assigner,
                    db_sampler=None,
...
def _read_and_prep_nu(info, root_path, num_point_features, prep_func):
    """read data from nusc-format infos, then call prep function.
    """

    v_path = str(root_path+info['lidar_path'])

    points = np.fromfile(
        str(v_path), dtype=np.float32,
        count=-1).reshape([-1, num_point_features])
    points[:, 3] /=255 #intensity =0~255  need to normalize
    points = points[:,:4]
    gt_names = info['gt_names']
    gt_boxes = info['gt_boxes']
    difficulty = np.zeros(gt_boxes.shape[0], dtype=np.int32)
    image_idx = np.arange(gt_boxes.shape[0],dtype=np.int32)
    input_dict = {
        'points': points,
        'group_ids':image_idx,
        'gt_boxes': gt_boxes,
...
def _read_and_prep_ikg(info, root_path, num_point_features,prep_func):
    """prepare data from IKG-format infos, call prep function
    """
    #root_path = ./data/IKG/object
    #info['velodyne_path'] = training/velodyne/xxx.pcd
    v_path = pathlib.Path(root_path) / info['velodyne_path']
    if 'velodyne_reduced' in info['velodyne_path']:
        points  = np.fromfile(str(v_path), dtype=np.float32, count=-1).reshape([-1, 4])
         # I have normalized intensity when creating reduced .bin

...
#####update 2021.05.07#######
def create_groundtruth_database_nu(data_path,
                                info_path=None,
                                used_classes=None,
    ...
    if info_path is None:
        info_path = root_path / 'nu_infos_val.pkl'
    if database_save_path is None:
        database_save_path = root_path / 'gt_databaseval'
    else:
        database_save_path = pathlib.Path(database_save_path)
    if db_info_save_path is None:
        db_info_save_path = root_path / "nu_dbinfos_valid.pkl"
    database_save_path.mkdir(parents=True, exist_ok=True)
    with open(info_path, 'rb') as f:
        nu_infos = pickle.load(f)

    all_db_infos = {}

    if used_classes is None:
        used_classes = list(class_to_label().keys())

    group_counter = 0
    for image_idx ,info in enumerate(nu_infos):
        velodyne_path = info['lidar_path']
        #./data + /second.pytorch/Test_nuscenes/samples/LIDAR_TOP/n015-2018-07-24-11-22-45+0800__LIDAR_TOP__1532402927647951.pcd.bin
        velodyne_path = str(data_path+velodyne_path)
        points = np.fromfile(velodyne_path,dtype=np.float32,count=-1).reshape([-1,4])

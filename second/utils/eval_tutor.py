...
def num_tp_ped(gt_annos,dt_annos,current_class):
    
    assert len(gt_annos) == len(dt_annos)
    #num_examples = len(gt_annos)
    # [2,2,2,2...1] include 50 * num_examples//50 ...1
    #split_parts = get_split_parts(num_examples, 50)

    rets = calculate_iou_partly(dt_annos, gt_annos, 50)
    overlaps, parted_overlaps, total_dt_num, total_gt_num = rets
